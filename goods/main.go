...
import (
	"github.com/Lemon-CS/go-lemon/goods/model"
...
func main() {
	...
	//engine.Use(lime.Tracer("goodsService", &config.SamplerConfig{
	...
	//}
	//defer closer.Close()
	//group.Get("/findTracer", func(ctx *lime.Context) {
	...
	//})
	//
	//group.Get("/find", func(ctx *lime.Context) {
	...
	group.Post("/find", func(ctx *lime.Context) {
		goods := &model.Goods{Id: 1000, Name: "9002的商品"}
		ctx.JSON(http.StatusOK, &model.Result{Code: 200, Msg: "success", Data: goods})
	})
